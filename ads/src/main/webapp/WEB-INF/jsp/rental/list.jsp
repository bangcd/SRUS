		...
		<section class="container section section-first">
			<div class="row justify-content-center">
				...
				<header class="col-12 col-lg-12 col-xl-10 post-header">
					...
					<h1 class="title title--h2 js-lines">내가 대여한 무인대여함</h1>
				</header>
				<table class="table">
					<tr>
						<th>대여함 번호</th>
					...
					<c:forEach var="item" items="${rows}">
						<tr>
							<td>${item.rentalBoxNo}번 대여함</td>
							<td>${item.paymentDate}</td>
							<td><fmt:formatNumber value="${item.amountOfPayment}"/>원</td>
							<td>${item.startDate}</td>
							<td>${item.endDate}</td>
							<td>
								<button type="button" class="btn btn__small w-20"
									onclick="location.href='/authentication/active?id=${sessionScope.id}&no=${item.rentalBoxNo}' ">인증
									활성화</button>
							</td>
						</tr>
					</c:forEach>
